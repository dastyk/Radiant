...
EnemyBasicProjectile::EnemyBasicProjectile(Entity enemyEntity, EntityBuilder* builder, XMFLOAT3 parentColor, XMFLOAT3 &playerPosition) : Projectile(builder)
{
	...
	_builder->Light()->BindPointLight(_projectileEntity, XMFLOAT3(0, 0, 0), 0.35f, XMFLOAT3(parentColor.x*1.3f, parentColor.y*1.3f, parentColor.z*1.3f), _lifeTime);
	XMFLOAT3 temp;
...
void EnemyBasicProjectile::Update(float deltaTime)
{
	...
	{
		_builder->Transform()->MoveAlongVector(_projectileEntity, XMLoadFloat3(&_movementVector)*13*deltaTime);
