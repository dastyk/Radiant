...
Audio::Audio(wchar_t directoryPath[200])
{
	HRESULT result;
...
HRESULT Audio::PlayGlobalSound(wchar_t* filename, float volume)
{
...
HRESULT Audio::PlayBGMusic(wchar_t * filename, float volume)
{
	if (pMusicVoice != nullptr)
	{
		pMusicVoice->DestroyVoice();
	...
	pMusicVoice->SetVolume(masterVolume * musicVolume * volume);
