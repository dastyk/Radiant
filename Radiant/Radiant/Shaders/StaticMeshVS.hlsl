...
VS_OUT VS( VS_IN input )
{
	...
	output.Normal = mul(float4(input.TangL, 0.0f), gWorldViewInvTrp);
	output.Tangent = mul(float4(input.BinoL, 0.0f), gWorldView);
	output.Binormal = mul(float4(input.NormL, 0.0f), gWorldView);
